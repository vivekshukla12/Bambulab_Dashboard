...
| Bambu read-only adapter | `packages/adapter-bambu-readonly` | adapter API, domain | M2 standard-mode local MQTTS read-only status adapter, bounded server-side discovery, parser, freshness and mocked-transport tests. |
