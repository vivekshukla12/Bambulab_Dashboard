...
For M2 real-printer onboarding, the Fleet setup panel automatically starts bounded server-side SSDP discovery when no configured real printer is present. Use the rescan action to refresh candidates; discovered entries return sanitized labels only and keep raw endpoint details on the server. Enter a real LAN Access Code in the browser form only from `localhost`/loopback on the same server machine or from an HTTPS-served dashboard. Use the local CLI validation config instead of the browser form when operating over remote LAN HTTP.
...
4. Optionally use the dashboard's automatic server-side SSDP discovery/rescan in the Fleet panel to identify candidates, then keep private hosts/serials/Access Codes only in the ignored local config.
